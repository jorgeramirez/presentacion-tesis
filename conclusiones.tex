...
\begin{table}[ht]
  \begin{tabular}{|p{2.75cm}|p{8cm}|}
    ...
    Tema & Conclusi\'on \\
    \hline
    \multirow{3}{2.75cm}{\uncover<2-4>{\textbf{Implementaci\'on de la Interfaz}}} & \uncover<2-4>{Evaluar las herramientas de reconocimiento del \mbox{habla} disponibles teniendo en cuenta criterios como el conocimiento t\'ecnico y los recursos econ\'omicos necesarios, la productividad y la flexibilidad}\\
    \hhline{~-}
    & \uncover<3-4>{Definir una arquitectura inspirada en servicios para mitigar problemas de integraci\'on entre los componentes.} \\
    \hhline{~-}
    & \uncover<4-4>{Realizar pruebas con usuarios de manera frecuenta para evaluar el dise\~no y la implementaci\'on.}\\
    \hline
  \end{tabular}
\end{table}
